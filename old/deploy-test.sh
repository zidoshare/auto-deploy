...
if [ ! -e "${localLogDir}/pkg" ]
then
  mkdir -p "${localLogDir}/pkg"
fi
...
packageProject(){
  ...
  pkgLogLocation="${localLogDir}/pkg/$3.log"
  `nohup ${packageCommand} > ${pkgLogLocation} 2>&1 &`
  sleep 1s
  while [ -f ${pkgLogLocation} ]
  do
    pkgResult=$(grep  "BUILD SUCCESS" "${pkgLogLocation}")
    if [ ! -z "${pkgResult}" ]
    then
      echo "package success"
      break
    else
      errorResult=$(grep "ERROR" "${pkgLogLocation}")
      if [ ! -z "${errorResult}" ]
      then
        echo ${errorResult}
  ...
  if [ ! -e "${jarLocation}" ]
  then
    echo "package success, but can't find the jar."
    exit 1
  fi
  # copy original jar
...
startup(){
  ...
  $(nohup "${localJavaBin}" -Xms64m -Xmx128m -jar "${localBinDir}/$1.jar"  > "${localLogDir}/$1.log"  2>&1 &)
  sleep 1s
  today=`date +%Y-%m-%d`
  hour=`date +%H`
